...
export function VaultTable() {
  ...
  return (
    <div className="stone-slab border-2 border-border p-0 overflow-hidden">
      ...
      {/* Mobile Card List (< md) */}
      <div className="block md:hidden divide-y divide-[#47483c]/30 font-mono">
        {sortedCredentials.length === 0 ? (
          <div className="p-8 text-center text-muted-foreground/60 uppercase tracking-widest text-xs">
            No matching secrets secured in database.
          </div>
        ) : (
          sortedCredentials.slice(0, visibleLimit).map((cred) => (
            <div
              ...
              className={`p-4 space-y-3 transition-colors ${
                expandedId === cred.id ? "bg-[#353534]/50" : ""
              }`}
            >
              <div
                onClick={() => toggleExpand(cred.id)}
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-8 h-8 bg-background border border-border flex items-center justify-center text-[#ffb77d] font-headline select-none shrink-0 text-sm">
                    {cred.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <div className="font-bold text-muted-foreground truncate hover:text-[#ffb77d] transition-colors text-xs">
                      {cred.name}
                    </div>
                    <div className="text-muted-foreground/70 text-[11px] truncate">
                      {cred.username}
                    </div>
                  </div>
                </div>
                ...
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-mono border shrink-0 ${
                    cred.strength === "Strong"
                      ? "bg-[#4b5320]/20 border-[#c3cc8c] text-[#c3cc8c]"
                      : cred.strength === "Medium"
                        ? "bg-[#fd8b00]/10 border-[#ffb77d] text-[#ffb77d]"
              ...
              {/* Password Bar */}
              <div className="flex items-center justify-between bg-input/40 border border-border/50 px-3 py-1.5 text-xs">
                <span className="tracking-widest font-bold text-muted-foreground select-none text-xs truncate mr-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <Button
                    ...
                    variant="ghost"
                    size="icon-xs"
                    className="text-muted-foreground/60 hover:text-[#ffb77d] hover:bg-transparent h-7 w-7"
                  >
                    {revealedIds.includes(cred.id) ? (
                      <EyeOff className="w-3.5 h-3.5" />
                    ) : (
              ...
              <AnimatePresence>
                {expandedId === cred.id && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, height: "auto" }}
                    exit={{ opacity: 0, height: 0 }}
                    className="overflow-hidden space-y-3 pt-2 text-xs"
                  >
                    ...
                    <div className="flex flex-wrap justify-end gap-2 pt-2 border-t border-border/30">
                      <Button
                        onClick={() =>
                          triggerCopy(cred.username, cred.id, "username")
                        }
                        variant="outline"
                        className="h-7 bg-card border-border text-xs font-mono rounded-none"
                      >
